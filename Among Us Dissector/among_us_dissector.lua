...
function V2index (index)
    local start_index = math.floor(index)
    return string.sub(V2, start_index + 1, start_index + 1)
end
    ...
    function object:decode_packed()
        ...
        do
            ...
            if currbyte >= 0x80 then
                currbyte = bit.bxor(currbyte, 0x80)
                output = bit.bor(output, bit.lshift(currbyte, shift))
                shift = shift + 7
            else
                output = bit.bor(output, bit.lshift(currbyte, shift))
                break
...
function decode_gamecode (gamecode)
    local a = bit.band(gamecode, 0x3FF)
    local b = bit.band(bit.rshift(gamecode, 10), 0xFFFFF)
...
function parseSpawnObjectComponents(buffwrap, pinfo, tree, objecttype, index)
    ...
    if objecttype == Spawned_Object_Ids["ShipStatus"] then
        ...
        for i=1, component_length:le_uint(), 1 do
            ...
            local reported_player_id = bit.band(voteAreaFlag:uint(), 0x0F) - 1
            local did_report = bit.band(voteAreaFlag:uint(), 0x20)
            local did_vote = bit.band(voteAreaFlag:uint(), 0x40)
            local is_dead = bit.band(voteAreaFlag:uint(), 0x80)
        ...
        if index == 1 then
            ...
            for i=1, player_count, 1 do
                ...
                local deathFlag = bit.band(flagbitfield:uint(), 4)
                local impostorFlag = bit.band(flagbitfield:uint(), 2)
                local disconnectFlag = bit.band(flagbitfield:uint(), 1)
